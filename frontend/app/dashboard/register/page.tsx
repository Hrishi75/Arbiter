...
export default function RegisterPage() {
  ...
  function handleContinue() {
    ...
    router.push(`/dashboard/agents?q=${encodeURIComponent(name.trim())}`);
  }
              ...
              {steps.map((step) => (
                <li
                  ...
                  className={cn(
                    ...
                    step.status === "in_progress" && "bg-surface-alt"
                  )}
                >
                  <span
                    className={cn(
                      "flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full text-[10px] font-mono",
                      step.status === "complete" && "bg-foreground text-background",
                      step.status === "in_progress" && "bg-accent text-accent-foreground",
                      step.status === "pending" &&
                        "border border-hairline-strong text-muted-foreground"
                    )}
                  >
                    {step.status === "complete" ? <Check className="h-3 w-3" /> : step.n}
                  </span>
            ...
            <div className="border border-hairline bg-card">
              ...
              <dl className="divide-y divide-hairline">
                ...
                <div className="flex justify-between px-4 py-3 text-xs">
                  ...
                  <dd className="font-mono font-medium text-accent">{youLock} ETH</dd>
                </div>
              </dl>
            </div>
          </aside>
